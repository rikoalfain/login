<?php
	...
	if($btnsimpan)
	{
		...
		$move=move_uploaded_file($_FILES['gambar']['tmp_name'],"../images/$fname");
				...
				if($move){
				$query=$db->prepare("insert into tbbarang values('$id','$nama','$id_kategori','$harga','$jumlah','$insertdate','','/images/$fname')");
		$query->execute();
		echo"<script>alert('sudah disimpan !!');location.href='index.php?p=tbbarang'
		</script>";
				}
		}
